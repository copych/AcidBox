...
#include "config.h" // M5Unified.h is included in config.h
#include "fx_delay.h"
#ifndef NO_PSRAM
#include "fx_reverb.h"
#endif
...
void setup(void) {
  ...
  auto cfg = M5.config();
  // cfg.serial_baudrate = 115200; // Example: Set baudrate if needed, default is 115200
  // cfg.output_power = true; // For M5Stack Core S3 with speaker, ensures power to AW88298.
  M5.begin(cfg);
...
// static bool led_state = false; // Removed for blink test cleanup
...
void loop() { // default loopTask running on the Core1
  // Restored original loop content
  regular_checks();
